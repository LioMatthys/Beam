...
## Driving Beam from an AI agent

While Beam is connected, the Windows app exposes a localhost JSON-RPC relay on
`127.0.0.1:8788`. Point any agent (or script) at it. The fastest way: in the Windows app's
**☰ menu**, click **"Copy instruction for agent"** and paste it into your agent. That prompt is:

```
You can see and control a real Android phone through Beam.
...
OPS
  ...
  screenshot {"maxLongSide"?}                   ->  {"png":"<base64>"}  (VISION fallback when dump is empty)
...
For a higher-level surface (typed methods, recorded flows, assertions), use
[DroidPilot](https://github.com/LioMatthys/droidpilot) — it speaks this exact protocol as an MCP server.
